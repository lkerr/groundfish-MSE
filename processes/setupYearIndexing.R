...
setupYearIndexing <- function(nyear,
                              fyear,
                              nrep){
  # Define economic years
  eyears <- nyear-fyear+1
  
  # Setup economic replicates and year indices for nreps
  econ_replicate <- rep(1:nrep,each=eyears)
  sim_year_idx <- rep(fyear:nyear, times=nrep)
  
  # Setup random_sim_draw - a table to contain different indexing
  ...
  #!!! Check that the above stops and prints to screen now that this code is part of a function
  
  # Return
  yearIndexing <- NULL
  
  yearIndexing$random_sim_draw <- random_sim_draw # A table containing the following indices in columns: econ_replicate, sim_year_idx, cal_year, manage_year_idx, join_econbase_yr, join_econbase_idx, join_outputprice_idx, join_inputprice_idx, join_mult_idx
  yearIndexing$yearitercounter <- yearitercounter # An iteration counter for year beginning at 0
  yearIndexing$max_yiter <- max_yiter # A number for the maximum number of year iterations
  yearIndexing$interpb <- interpb # Function name that generates progress bar
