...
get_popInit <- function(stock){
  ...
  stock <- within(stock, {
    ...
    # in the first n fyear years.
    
    initN <- get_init(nage=nage, N0=2e7, F_full=F_full[1], M=M)
    J1N[1:fyear,] <- rep(initN, each=fyear)
    
    laa[1:fyear,] <- rep(get_lengthAtAge(type='vonB', par=laa_par, 
                                             ages=fage:page, Tanom=0),
                             each=fyear)
    waa[1:fyear,] <- get_weightAtAge(type='aLb', par=waa_par, 
                                         laa=laa[1:fyear,],
                                         inputUnit='kg')
    Z[1:fyear,] <- rep(F_full[1]+M, each=fyear)
