#Bulk of the OM
#Bulk of the OM
get_J1Updates <- function(stock,y,assess_vals){
  ...
  out <- within(stock, {
    ...
    if (y < fmyearIdx){
      Rout <- get_recruits(type=R_typ, type2='True', par=Rpar, S=SSB[y], block = 'early',
                           ...
                           Rhat_ym1 = Rhat[y-1],R_est=parpop$R,y=y)
    }
    if (y >= fmyearIdx){
    ...
    if (histAssess == TRUE) {
      for(i in 1:nstock){
        if(y %in% assess_vals$assessdat$MSEyr){
          rep_assess <- get_AssessVals(y=y,assess_vals=assess_vals)
          F_full[y] <- rep_assess$fish_mort
          R[y] <- as.numeric(rep_assess$rec)
