...
get_fillRepArrays <- function(stock){
  ...
  out <- within(stock, {
    ...
    if(stockName %in% ASAPstocks){
    omval$FPROXY[r,m,] <- RPmat[,1]
    omval$SSBPROXY[r,m,] <- RPmat[,2]
    #omval$FPROXYT[r,m,] <- RPmat[,3]
    #omval$SSBPROXYT[r,m,] <- RPmat[,4]
    # omval$FRATIO[r,m,y] <- res$F.report/RPmat[,1][y]
    # omval$SSBRATIO[r,m,y] <- res$SSB/RPmat[,2][y]
    ...
    omval$SSBest[y,1:length(res$SSB)]<-res$SSB
    omval$Fest[y,1:length(res$F.report)]<-res$F.report
    omval$Catchest[y,1:length(res$catch.pred)]<-res$catch.pred
    omval$Rest[y,1:length(res$N.age[,1])]<-res$N.age[,1]
    # if (y == nyear){
