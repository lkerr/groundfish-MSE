get_hydra <- function(){
  ...
  source(here("functions/hydra/hydra_sim_data.R"))
  ...
  hydra_sim_rep <- reptoRlist("hydra_sim.rep")
  
  #Convert general report object into things to be fed back
  surveydata.df <- hydra_sim_rep$survey
  colnames(surveydata.df) <- c("Survey","Year","Species","Observed Biomass","CV?","Predicted Biomass","Residual","NLL")
  catchdata.df <- hydra_sim_rep$catch
  colnames(catchdata.df) <- c("Fleet","Area","Year","Species","Observed Biomass","CV?","Predicted Biomass","Residual","NLL")
