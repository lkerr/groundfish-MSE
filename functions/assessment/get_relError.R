...
get_relError <- function(stock){
  ...
  out <- within(stock, {
    ...
    if(mproc[m,'ASSESSCLASS'] == 'ASAP' & y > fmyearIdx-1){
      relE_SSB[y] <- get_relE(tail(res$SSB,1), SSB[y])
      relE_CW[y] <- get_relE(res$catch.pred[37], sumCW[y])
      relE_IN[y] <- get_relE(tail(res$index.pred$ind01,1), sumIN[y])
      relE_qI[y] <- get_relE(log(res$q.indices), log(qI))
      ...
      relE_R[y] <- get_relE(tail(res$SR.resids$recruits,1), R[y])
      relE_F[y] <- get_relE(tail(res$F.report,1), F_full[y])
    } 
  })
