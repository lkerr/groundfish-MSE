##################################################################################
### function to create ASAP .dat file, run executable, and produce results object
...
get_ASAP <- function(stock){
  ...
  out <- within(stock, {
    ...
    dat <- list(
      ...
      WAA_mats <- get_dwindow(waa, styear, endyear),
      WAA_pointers <- waa_pt,
      ...
      IAA_mats <- cbind(get_dwindow(obs_sumIN, styear, endyear), get_dwindow(obs_paaIN, styear, endyear), oe_paaIN), #value, CV, by-age, sample size
      phase_F1 <- 1,
      phase_F_devs <- 3,
      phase_rec_devs <- 2,
      phase_N1_devs <- 1,
      ...
      discard_cv <- matrix(0, nrow = nyear, ncol = nfleet),
      catch_Neff <- matrix(oe_paaCN, nrow = ncaayear, ncol = nfleet),
      discard_Neff <- matrix(0, nrow = nyear, ncol = nfleet),
