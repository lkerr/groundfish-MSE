...
get_ASAP <- function(stock){
  ...
  out <- within(stock, {
    ...
    dat_file$dat$IAA_mats <- cbind(seq(styear,endyear), get_dwindow(sumIN, styear, endyear), rep(oe_sumIN, N_rows), get_dwindow(paaIN, styear, endyear), rep(oe_paaIN, N_rows)) #year, value, CV, by-age, sample size
