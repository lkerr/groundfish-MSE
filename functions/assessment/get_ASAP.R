...
#browser()
   ...
   dat_file$dat$M <- matrix(M, nrow = N_rows, ncol = page)
    ...
    #maturity-at-age
    dat_file$dat$maturity <- matrix(mat_par, ncol=page, nrow=N_rows, byrow = TRUE)

    #WAA matrix
    dat_file$dat$WAA_mats <-matrix(waa_par, ncol=page, nrow = N_rows, byrow= TRUE)
    ...
    dat_file$dat$CAA_mats <- cbind(get_dwindow(paaCN,1,N_rows), get_dwindow(sumCW, 1, N_rows))
    ...
    dat_file$dat$IAA_mats <- cbind(seq(styear,endyear), get_dwindow(sumIN, 1, N_rows), rep(oe_sumIN, N_rows), get_dwindow(paaIN, 1, N_rows), rep(oe_paaIN, N_rows)) #year, value, CV, by-age, sample size
