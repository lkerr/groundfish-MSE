...
get_ASAP <- function(stock){
  ...
  out <- within(stock, {
    ...
    dat_file <- ReadASAP3DatFile(paste('assessment/ASAP/', stockName[[i]], ".dat", sep = ''))
...
#     }
   ...
   # dat_file$dat$sel_block_assign <- matrix(1, nrow = N_rows, 1)
    ...
    dat_file$dat$CAA_mats <- list(cbind(cbind(paaCN$one, paaCN$two,paaCN$three, paaCN$four, paaCN$five, paaCN$six, paaCN$seven, paaCN$eight, paaCN$nine, paaCN$ten), sumCW$catch))
  ...
  #  dat_file$dat$DAA_mats <- matrix(0, nrow = N_rows, ncol = page + 1)
   ...
   # dat_file$dat$prop_rel_mats <- matrix(0, nrow = N_rows, ncol = page)
    ...
    dat_file$dat$IAA_mats <- list(cbind(sumIN$year, sumIN$biomass, rep(0.05, 37),cbind(paaIN$one, paaIN$two, paaIN$three, paaIN$four, paaIN$five, paaIN$six, paaIN$seven, paaIN$eight, paaIN$nine, paaIN$ten), rep(0.05, 37))) #year, value, CV, by-age, sample size
