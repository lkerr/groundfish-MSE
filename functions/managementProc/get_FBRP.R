...
get_FBRP <- function(parmgt, 
                     parpop, 
                     parenv, 
                     # Rfun_lst, # No longer needed, just pull the value from parmgt (a row of mproc)
                     stockEnv){
...
#               
  ...
  # Rfun <- Rfun_lst[[parmgt$RFUN_NM]] # Provide a string rather than the function name
  ...
  if(parmgt$FREF_TYP == 'YPR' | parmgt$FREF_TYP == 'SPR'){
    ...
    if(parmgt$BREF_TYP == 'SIM' & parmgt$RFUN_NM == 'forecast'){
      ...
      simAtF <- get_proj(type = 'FREF',
                 ...
                 parpop = parpop, 
                 parenv = parenv, 
                 Rfun = parmgt$RFUN_NM, # Use R function specified as input in mproc (parmgt is a row of mproc)
                 F_val = F,
                 ny = 200,
                 stReportYr = 2,
                 stockEnv = stockEnv)
    ...
    slp <- get_replacement(parpop = parpop, parmgt = parmgt, Rfun_BmsySim = Rfun_lst) # ??? doesn't appear to use Rfun_BmsySim? may need updating
