...
get_FBRP_true <- function(stock,parenv){
  ...
  #range of F to simulate over
    ...
    yields<- matrix(NA, nrow=100,ncol=length(candF))
    
    for (k in 1:100){
    for (i in 1:126){
    for (j in y:210){
    # calculate length-at-age in year 
    laa[j,] <- get_lengthAtAge(type=laa_typ, par=laa_par,
                               ages=fage:page, Tanom=Tanom[j])
    
    # calculate weight-at-age in year y
    waa[j,] <- get_weightAtAge(type=waa_typ, par=waa_par,
                               laa=laa[j,], inputUnit='kg')
    
    # calculate maturity in year y
    mat[j,] <- get_maturity(type=mat_typ, par=mat_par, laa=laa[j,])
    
    # calculate recruits in year y based on the SSB in years previous
    # (depending on the lag) and temperature
    SSB[j] <- sum(J1N[j-fage,] * mat[j,] * waa[j-fage,])
    ...
    slxC[j,] <- get_slx(type=selC_typ, par=selC, laa=laa[j,])
    slxI[j,] <- get_slx(type=selI_typ, par=selI, laa=NULL)
    ...
    CN[j,] <- get_catch(F_full=candF[i], M=natM[j],
                        N=J1N[j,], selC=slxC[j,]) + 1e-3
    
    # get Z for the current year
    Z[j,] <- candF[i]*slxC[j,] + natM[j]
    }
    yields[k,i]<-sum(CN[210,]*waa[210,])
    }}
