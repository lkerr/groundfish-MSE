# Function to compute an absolute Theil Index
...
get_absTheil=function(dataset, y){
  y2<-as.name(y)
  nobs<-nrow(dataset)
  working<-dataset  %>%
    dplyr::filter(!!y2>0) %>%
    summarise(Thiel=sum(!!y2*log(!!y2)/nobs))
...
# 
# T<-lapply(yvars, FUN=get_absTheil, dataset=test_data) 
# 
# T1
# T2
# T
