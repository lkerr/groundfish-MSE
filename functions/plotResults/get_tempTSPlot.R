...
get_tempTSPlot <- function(tanom, yrs, fmyear, tanomStd){
  
  tanomrg <- range(tanom)
  
  par(mar=c(4,4,1,1))
  
  plot(tanom ~ yrs, type='n', ann=FALSE, las=1, axes=FALSE)
  
  rect(xleft=yrs[1], 
       ybottom=tanomrg[1]-(tanomrg[2]-tanomrg[1])*0.025, 
       xright=yrs[which(tanom!=0)[1]], 
       ytop=tanomrg[1]+(tanomrg[2]-tanomrg[1])*0.025, 
       col='lightblue', border='gray20')
  
  rect(xleft=yrs[which(tanom!=0)[1]], 
       ybottom=tanomrg[1]-(tanomrg[2]-tanomrg[1])*0.025, 
       xright=yrs[fmyear], 
       ytop=tanomrg[1]+(tanomrg[2]-tanomrg[1])*0.025, 
  ...
  lines(tanom ~ yrs, type='o', pch=16, cex=0.5, las=1, col='gray20')
  abline(h=0, lty=3)
  box()
  
  xaxlab <- pretty(yrs, 5)
  yaxlab <- pretty(tanom, 5)
