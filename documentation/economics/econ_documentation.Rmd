...
To be reasonably accurate, a multispecies simulation model must account for technical interactions. We do that for the northeast US groundfish fishery.  This allows for fishing mortality to be systematically different from the level set by fishery managers.
...
Single species (or single stock) management is common in the US. Fishery managers select F or an F-control rule at the stock level based on legal constraints and scientific advice; the fishing mortality rate is operationalized into an annual catch limit.  Legal constraints in the US include mandates to achieve Optimum Yield or rebuild overfished stocks of fish.  Scientific advice includes both estimates of current stock levels and future stock levels (that are functions of different levels of F).  This requires some assumptions about whether catch will be greater than, less than, or equal to the catch limit.  When a stock of fish is not caught jointly with other stocks, the catch limits are "low" enough to matter, and the manager has enough information to effectively monitor and close the fishery when the catch limit is approached, the assumption that catch is equal to the catch limit, perhaps with some small error, is reasonable.  In the Northeast US, this situation might be true for Atlantic Herring and Atlantic Sea Scallops.

When the manager cannot acquire information to monitor in real-time, a fishing fleet's catch can be greater than it's catch limit.  The recreational cod fishery in the Gulf of Maine is an example: catch is estimated using a survey and catch statistics are often not available until well after the end of the fishing year.  When two stocks of fish are caught jointly, the catch limit of one species may constrain a fishing fleet's catch of a second to be less than the catch limit.  The extent to which catch limits of one stock constrain catch of the other stock depends on stock conditions and catch limits of both stocks, technology used to capture those stocks, and the strength of the incentives to catch or avoid each stock. When these factors change, the relationship between the constrained and constraining stocks are likely to change as well. Finally, when catch limits are "high" catch will be less than the limit; this might occur if there is market power at some stage in the production process -- businesses may be able to deter entry of new vessels or lower catch in order to keep output prices high. It may also occur if output prices are too low or input prices are too high to incentivize firms to enter or catch fish. 

These processes are difficult to model mechanistically and phenomenological approaches are often used to model F [@Fenichel2014; @Abbott2013]. 
