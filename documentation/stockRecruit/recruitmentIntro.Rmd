...
### Data
Read in the stock and recruitment data. These are organized in a single file and the source of the data (e.g., SAW55) are included. Stock size and recruits are in the same year so for the analysis the data will need to be offset (i.e., age-1 recruitment in 2014 should be a function of SSB in 2013). Note that the working directory of this document is the file location hence the need for the '../' at the start of the file path. Possibly the best way to do this would be adaptively within the stock recruitment function -- that way the number of years to offset could be generalized but this is a little clearer.
...
To ensure this is doing what we want let's look at the start of both versions of the data files
