...
calc_metrics <- function(results, yrs = 1:10) {
  ...
  #cat_yield 
  fleet_yield <- results$fleet_catch %>% 
    dplyr::filter(year %in% yrs) %>% 
  ...
  prop_below <- results$sp_results %>% 
    ...
    I()
  #prop_below
  prop_overfishing <- results$sp_results %>% 
    dplyr::filter(year %in% yrs) %>% 
    group_by(rep, species) %>% 
    ...
    I()
  #prop_overfishing
  eco_above_bmsy <- results$sp_results %>% 
    dplyr::filter(year %in% yrs,
                  species == 14) %>% 
    group_by(rep) %>% 
