...
ASAPstocks <-c(1,2,3,5,8)
for (i in ASAPstocks) {
  data <- stock[[i]]
  om_biomass <- as.data.frame(cbind(data$SSB,seq(1:62)))
  om_biomass<-dplyr::filter(om_biomass, V2<62)
  assess_biomass <- as.data.frame(cbind(data$parpop$SSBhat, seq(1:62)))
...
for (i in ASAPstocks) {
  data <- stock[[i]]
  om_biomass <- as.data.frame(cbind(data$SSB,seq(1:61)))
  om_biomass<-dplyr::filter(om_biomass, V2<61)
  colnames(om_biomass)<-c('om_biomass','Year')
  assess_biomass <- as.data.frame(cbind(data$parpop$SSBhat, seq(1:61)))
...
PlanBstocks <-c(4,6,7,9,10)
for (i in PlanBstocks) {
  data <- stock[[i]]
  om_catch <- as.data.frame(cbind(data$planBdata$avg,seq(1:61)))
  om_catch<-dplyr::filter(om_catch, V2<61)
  colnames(om_catch)<-c('om_biomass','Year')
  assess_catch <- as.data.frame(cbind(data$planBest$pred$fit, c(20:61)))
  colnames(assess_catch)<-c("assess_biomass", 'Year')
