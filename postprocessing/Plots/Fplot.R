...
for (k in 1:length(sims)){
  load(sims[k])
  Catchsim[,k]<-omvalGlobal[[1]]$F_full[137:189]
}
...
for (i in 1:length(files)){
  ...
  Catch[,i]<-tryCatch(readRDS(paste(Stock,'_1_190.rdat',sep=""))$F.report,error=function(err) NA)
}
...
ggplot(df)+geom_line(aes(x=Year,y=Catchest,color=HCR))+geom_point(aes(x=Year,y=Catchsim,color=HCR))+
  ...
  ylab('F')+geom_vline(xintercept=2020, linetype='dotted')+
  scale_color_manual(values=c("#EA4F12","#EACA00","#407331","#00608A"))+  
  scale_y_continuous(breaks = pretty(c(0,2.5), n=6),limits = c(0,2.5))
