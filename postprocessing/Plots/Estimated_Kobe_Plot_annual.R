...
for (k in 1:length(sims)){
  ...
  Fproxy[,k]<-omvalGlobal[[1]]$FPROXY[169:189]
  SSBestproxy[,k]<-omvalGlobal[[1]]$SSBPROXY[169:189]
}
...
kobe <- kobe + annotate(geom = "rect", xmin = 1, xmax = maxSSBest, ymin = 0, ymax = 1, fill = "green", colour = "green", alpha = 0.5) +
  ...
  geom_text_repel(data=subset(df, Year > 2038 | Year < 2020),aes(x = SSBestratioreal, y = Fratioreal, label = Year))
kobe
