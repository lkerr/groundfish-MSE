...
for (k in 1:length(sims)){
  load(sims[k])
  Catchsim[,k]<-omvalGlobal[[1]]$R[137:189]
}
...
for (i in 1:length(files)){
  ...
  Catch[,i]<-tryCatch(readRDS(paste(Stock,'_1_190.rdat',sep=""))$N.age[,1],error=function(err) NA)
}
...
ggplot(df)+geom_line(aes(x=Year,y=Catchest,color=HCR))+geom_point(aes(x=Year,y=Catchsim,color=HCR))+
  ...
  ylab('Recruits')+geom_vline(xintercept=2020, linetype='dotted')+
