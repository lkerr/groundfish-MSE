...
box_types<-c("F_full_2025-2030", "F_full_2030-2040", "SSB_2025-2030", "SSB_2030-2040")

sim_traj_types<-c("Shannon_fleet", "total_modeled_rev", "HHI_fleet", "Gini_fleet")
sim_box_types<-c("Shannon_fleet_2025-2030","Shannon_fleet_2030-2040", "total_modeled_rev_2025-2030", "total_modeled_rev_2030-2040")
...
ds<-NULL
# Entries in OMVAL are stored in ARRAYS.  The first indexes replicates i=1:nrep. The second indexes mproc, m=1:nrow(mproc). THe 3rd indexes years y=yr1:mxyear.
...
for(i in 1:length(omvalGlobal)){
  ...
  ie_F_hat<-omvalGlobal[[i]]$ie_F_hat
  ie_F_hat<-as_tibble(cbind(expand.grid(dim1,  dim2), ie_F_hat = as.vector(ie_F_hat)))
  ie_F_hat<-ie_F_hat %>%
  dplyr::rename(replicate=Var1,
                mproc=Var2) 

  ie_F<-omvalGlobal[[i]]$ie_F
  ie_F<-as_tibble(cbind(expand.grid(dim1,  dim2), ie_F = as.vector(ie_F)))
  ie_F<-ie_F %>%
  dplyr::rename(replicate=Var1,
                mproc=Var2) 

  ie_bias<-omvalGlobal[[i]]$ie_bias
  ie_bias<-as_tibble(cbind(expand.grid(dim1,  dim2), ie_bias = as.vector(ie_bias)))
  ie_bias<-ie_bias %>%
  dplyr::rename(replicate=Var1,
                mproc=Var2) 

  iebias_hat<-omvalGlobal[[i]]$iebias_hat
  iebias_hat<-as_tibble(cbind(expand.grid(dim1,  dim2), iebias_hat = as.vector(iebias_hat)))
  iebias_hat<-iebias_hat %>%
  dplyr::rename(replicate=Var1,
                mproc=Var2) 

  
  #Just pick the index from one of the model runs
  ACL<-omvalGlobal[[i]]$ACL
  ACL<-as_tibble(cbind(expand.grid(dim1,  dim2, dim3), ACL = as.vector(ACL)))
  ...
  SSB<-omvalGlobal[[i]]$SSB
  SSB<-as_tibble(cbind(expand.grid(dim1,  dim2, dim3), Simulated_SSB = as.vector(SSB)))
  ...
  F_fullAdvice<-omvalGlobal[[i]]$F_fullAdvice
  F_fullAdvice<-as_tibble(cbind(expand.grid(dim1,  dim2, dim3), F_fullAdvice = as.vector(F_fullAdvice)))
  
  F_fullAdvice<-F_fullAdvice %>%
  dplyr::rename(replicate=Var1,
                mproc=Var2,
                year=Var3) 

  
  F_full<-omvalGlobal[[i]]$F_full
  F_full<-as_tibble(cbind(expand.grid(dim1,  dim2, dim3), F_full = as.vector(F_full)))
  ...
  sumCW<-omvalGlobal[[i]]$sumCW
  sumCW<-as_tibble(cbind(expand.grid(dim1,  dim2, dim3), sumCW = as.vector(sumCW)))
  ...
  OFdStatus<-omvalGlobal[[i]]$OFdStatus
  OFdStatus<-as_tibble(cbind(expand.grid(dim1,  dim2, dim3), OFdStatus = as.vector(OFdStatus)))
  ...
  OFgStatus<-omvalGlobal[[i]]$OFgStatus
  OFgStatus<-as_tibble(cbind(expand.grid(dim1,  dim2, dim3), OFgStatus = as.vector(OFgStatus)))
  ...
  extracted <-dplyr::inner_join(ACL, SSB, by=c("replicate", "mproc", "year"))
  extracted <-dplyr::inner_join(extracted, F_fullAdvice, by=c("replicate", "mproc", "year"))
  extracted <-dplyr::inner_join(extracted, F_full, by=c("replicate", "mproc", "year"))
  extracted <-dplyr::inner_join(extracted, sumCW, by=c("replicate", "mproc", "year"))
  extracted <-dplyr::inner_join(extracted, OFdStatus, by=c("replicate", "mproc", "year")) 
  extracted <-dplyr::inner_join(extracted, OFgStatus, by=c("replicate", "mproc", "year")) 
...
ds<-ds %>%
  ...
  mutate(ACL=ACL/1000) %>%
  mutate(Simulated_SSB=Simulated_SSB/1000) %>%
...
for(i in 1:length(omvalGlobal)){
  ...
  model_level <-dplyr::inner_join(ie_F_hat, ie_F, by=c("replicate", "mproc")) 
  model_level <-dplyr::inner_join(model_level, ie_bias, by=c("replicate", "mproc")) 
  model_level <-dplyr::inner_join(model_level, iebias_hat, by=c("replicate", "mproc")) 
  model_level$stock<-names(omvalGlobal[i])

  ml<-rbind(ml, model_level)
  model_level<-NULL
}  
  # not automated, but 
...
```
I simulated from 2020-2040 using an economic model (MP1) and the standard fisheries (MP2).  For the Economic model, I have set the economic parameters (prices, daily weather, catch limits and trawl index, costs) equal to their 2015 actual values.  For the five stocks with an biological model, I have printed a handful of images.
...
```{r codGB_traj, eval=TRUE, fig.show = "hold", out.width = "48%", fig.align = "center", echo=FALSE, fig.cap="\\label{figure:codGB_traj} Select GB Cod Mean/Median.  MP1=Economic, MP2=Standard Fisheries"}
...
```{r codGB_box, eval=TRUE, fig.show = "hold", out.width = "48%", fig.cap="\\label{codGB_box} GB Cod box plots for F\\_Full (top) and  SSB (bottom).  MP1=Economic, MP2=Standard Fisheries",  fig.align = "center", echo=FALSE}
...
```{r plot_GBcod_catch,  fig.cap="GB cod Catch"}
 ds2 %>%dplyr::filter(stock=="codGB") %>% dplyr::filter(mproc!=3) %>% ggplot(data=., aes(x=year, y=sumCW, group=mproc, color=mproc)) +
  geom_line() + 
...
```{r plot_GBcod_ACL,  fig.cap="GB cod ACL"}
 ds2 %>%dplyr::filter(stock=="codGB") %>% dplyr::filter(mproc!=3) %>% ggplot(data=., aes(x=year, y=ACL, group=mproc, color=mproc)) +
  geom_line() + 
  facet_wrap(vars(replicate))#+ 
#  theme(legend.position = "none")
...
```{r plot_GBcod_SSB,  fig.cap="GB cod SSB"}
 ds2 %>%dplyr::filter(stock=="codGB") %>% dplyr::filter(mproc!=3) %>% ggplot(data=., aes(x=year, y=Simulated_SSB, group=mproc, color=mproc)) +
  geom_line() + 
  facet_wrap(vars(replicate))#+ 
#  theme(legend.position = "none")
...
```{r plot_GBcod_FFA,  fig.cap="GB cod SSB"}
 ds2 %>%dplyr::filter(stock=="codGB") %>% dplyr::filter(mproc!=3) %>% ggplot(data=., aes(x=year, y=F_fullAdvice, group=mproc, color=mproc)) +
  geom_line() + 
  facet_wrap(vars(replicate))#+ 
#  theme(legend.position = "none")
...
```{r Pollock_traj, eval=TRUE, fig.show = "hold", out.width = "48%",  fig.align = "center", echo=FALSE, fig.cap="\\label{fig:pollock_traj} Select Pollock  Mean/Median.  MP1=Economic, MP2=Standard Fisheries"}
...
```{r Pollock_box, eval=TRUE, fig.show = "hold", out.width = "48%", fig.cap="\\label{Pollock_box} Pollock box plots for F\\_Full (top) and  SSB (bottom).  MP1=Economic, MP2=Standard Fisheries",  fig.align = "center", echo=FALSE}
...
```{r plot_Pollock_catch,  fig.cap="GB cod Catch"}
 ds2 %>%dplyr::filter(stock=="pollock") %>% dplyr::filter(mproc!=3) %>% ggplot(data=., aes(x=year, y=sumCW, group=mproc, color=mproc)) +
  geom_line() + 
  facet_wrap(vars(replicate))#+ 
#  theme(legend.position = "none")
...
```{r plot_Pollock_ACL,  fig.cap="GB cod ACL"}
 ds2 %>%dplyr::filter(stock=="pollock") %>% dplyr::filter(mproc!=3) %>% ggplot(data=., aes(x=year, y=ACL, group=mproc, color=mproc)) +
  geom_line() + 
  facet_wrap(vars(replicate))#+ 
#  theme(legend.position = "none")
...
```{r plot_Pollock_SSB,  fig.cap="GB cod SSB"}
 ds2 %>%dplyr::filter(stock=="pollock") %>% dplyr::filter(mproc!=3) %>% ggplot(data=., aes(x=year, y=Simulated_SSB, group=mproc, color=mproc)) +
  geom_line() + 
  facet_wrap(vars(replicate))#+ 
#  theme(legend.position = "none")
...
```{r plot_Pollock_FFA,  fig.cap="GB cod SSB"}
 ds2 %>%dplyr::filter(stock=="pollock") %>% dplyr::filter(mproc!=3) %>% ggplot(data=., aes(x=year, y=F_fullAdvice, group=mproc, color=mproc)) +
  geom_line() +
...
```{r GBhaddock_traj, eval=TRUE, fig.show = "hold", out.width = "48%", fig.cap="\\label{GBhaddock} Select GB haddock Mean/Median. MP1=Economic, MP2=Standard Fisheries",  fig.align = "center", echo=FALSE}
...
```{r GBhaddock_box, eval=TRUE, fig.show = "hold", out.width = "48%", fig.cap="\\label{GBhaddock_box} GB Haddockbox plots for F\\_Full (top) and  SSB (bottom).  MP1=Economic, MP2=Standard Fisheries",  fig.align = "center", echo=FALSE}
...
```{r plot_GBhaddock_catch,  fig.cap="GB haddock Catch"}
 ds2 %>%dplyr::filter(stock=="haddockGB") %>% dplyr::filter(mproc!=3) %>% ggplot(data=., aes(x=year, y=sumCW, group=mproc, color=mproc)) +
  geom_line()  +
  theme(axis.text.x = element_text(angle = 45)) + 
  facet_wrap(vars(replicate))#+ 
#  theme(legend.position = "none")
...
```{r plot_haddockGB_ACL,  fig.cap="GB haddock ACL"}
 ds2 %>%dplyr::filter(stock=="haddockGB") %>% dplyr::filter(mproc!=3) %>% ggplot(data=., aes(x=year, y=ACL, group=mproc, color=mproc)) +
  geom_line()  +
  theme(axis.text.x = element_text(angle = 45)) +
  facet_wrap(vars(replicate))#+ 
#  theme(legend.position = "none")
...
```{r plot_haddockGB_SSB,  fig.cap="GB haddock SSB"}
 ds2 %>%dplyr::filter(stock=="haddockGB") %>% dplyr::filter(mproc!=3) %>% ggplot(data=., aes(x=year, y=Simulated_SSB, group=mproc, color=mproc)) +
  geom_line()  +
  theme(axis.text.x = element_text(angle = 45)) + 
  facet_wrap(vars(replicate))#+ 
#  theme(legend.position = "none")
...
```{r plot_haddockGB_FFA,  fig.cap="GB haddock SSB"}
 ds2 %>%dplyr::filter(stock=="haddockGB") %>% dplyr::filter(mproc!=3) %>% ggplot(data=., aes(x=year, y=F_fullAdvice, group=mproc, color=mproc)) +
  geom_line() +
  theme(axis.text.x = element_text(angle = 45)) + 
  facet_wrap(vars(replicate))#+ 
#  theme(legend.position = "none")
...
```{r GOMCod_traj, eval=TRUE, fig.show = "hold", out.width = "48%", fig.cap="\\label{GOMCod_traj} Select GOM Cod Mean/Median.  MP1=Economic, MP2=Standard Fisheries",  fig.align = "center", echo=FALSE}
...
```{r GOMCod_box, eval=TRUE, fig.show = "hold", out.width = "48%", fig.cap="\\label{GOMCod_box} GOM Cod box plots  for F\\_Full (top) and  SSB (bottom).  MP1=Economic, MP2=Standard Fisheries",  fig.align = "center", echo=FALSE}
...
```{r plot_codGOM_catch,  fig.cap="GOM Cod Catch"}
 ds2 %>%dplyr::filter(stock=="codGOM") %>% dplyr::filter(mproc!=3) %>% ggplot(data=., aes(x=year, y=sumCW, group=mproc, color=mproc)) +
  geom_line()  +
  theme(axis.text.x = element_text(angle = 45)) + 
  facet_wrap(vars(replicate))#+ 
#  theme(legend.position = "none")
...
```{r  plot_codGOM_ACL,  fig.cap="GOM Cod ACL"}
 ds2 %>%dplyr::filter(stock=="codGOM") %>% dplyr::filter(mproc!=3) %>% ggplot(data=., aes(x=year, y=ACL, group=mproc, color=mproc)) +
  geom_line()  +
  theme(axis.text.x = element_text(angle = 45)) + 
  facet_wrap(vars(replicate))#+ 
#  theme(legend.position = "none")
...
```{r  plot_codGOM_SSB,  fig.cap="GOM Cod SSB"}
 ds2 %>%dplyr::filter(stock=="codGOM") %>% dplyr::filter(mproc!=3) %>% ggplot(data=., aes(x=year, y=Simulated_SSB, group=mproc, color=mproc)) +
  geom_line()  +
  theme(axis.text.x = element_text(angle = 45)) +
  facet_wrap(vars(replicate))#+ 
#  theme(legend.position = "none")
...
```{r  plot_codGOM_FFA,  fig.cap="GOM Cod SSB"}
 ds2 %>%dplyr::filter(stock=="codGOM") %>% dplyr::filter(mproc!=3) %>% ggplot(data=., aes(x=year, y=F_fullAdvice, group=mproc, color=mproc)) +
  geom_line()  +
  theme(axis.text.x = element_text(angle = 45)) + 
  facet_wrap(vars(replicate))#+ 
#  theme(legend.position = "none")
...
```{r GBytf_traj, eval=TRUE, fig.show = "hold", out.width = "48%", fig.cap="\\label{GBytf} Select GB Yellowtail Mean/Median. MP1=Economic, MP2=Standard Fisheries",  fig.align = "center", echo=FALSE}
...
```{r GBytf_box, eval=TRUE, fig.show = "hold", out.width = "48%", fig.cap="\\label{GBytf_box} GB Yellowtail box plots  for F\\_Full (top) and  SSB (bottom). MP1=Economic, MP2=Standard Fisheries",  fig.align = "center", echo=FALSE}
...
```{r plot_yellowtailGB_catch,  fig.cap="yellowtailGB  Catch"}
 ds2 %>%dplyr::filter(stock=="yellowtailGB") %>% dplyr::filter(mproc!=3) %>% ggplot(data=., aes(x=year, y=sumCW, group=mproc, color=mproc)) +
  geom_line()  +
  theme(axis.text.x = element_text(angle = 45)) + 
  facet_wrap(vars(replicate))#+ 
#  theme(legend.position = "none")
...
```{r  plot_yellowtailGB_ACL,  fig.cap="yellowtailGB ACL"}
 ds2 %>%dplyr::filter(stock=="yellowtailGB") %>% dplyr::filter(mproc!=3) %>% ggplot(data=., aes(x=year, y=ACL, group=mproc, color=mproc)) +
  geom_line() +
  theme(axis.text.x = element_text(angle = 45)) + 
  facet_wrap(vars(replicate))#+ 
#  theme(legend.position = "none")
...
```{r  plot_yellowtailGB_SSB,  fig.cap="yellowtailGB SSB"}
 ds2 %>%dplyr::filter(stock=="yellowtailGB") %>% dplyr::filter(mproc!=3) %>% ggplot(data=., aes(x=year, y=Simulated_SSB, group=mproc, color=mproc)) +
  geom_line()  +
  theme(axis.text.x = element_text(angle = 45)) + 
  facet_wrap(vars(replicate))#+ 
#  theme(legend.position = "none")
...
```{r  plot_yellowtailGB_FFA,  fig.cap="yellowtailGB SSB"}
 ds2 %>%dplyr::filter(stock=="yellowtailGB") %>% dplyr::filter(mproc!=3) %>% ggplot(data=., aes(x=year, y=F_fullAdvice, group=mproc, color=mproc)) +
  geom_line()  +
  theme(axis.text.x = element_text(angle = 45)) + 
  facet_wrap(vars(replicate))#+ 
#  theme(legend.position = "none")
...
```{r SimLevel, eval=TRUE, fig.show = "hold", out.width = "48%", fig.cap="\\label{SimLevel} Simulation Level Figures MP1=Economic, MP2=Standard Fisheries",  fig.align = "center", echo=FALSE}
...
```{r SimLevelbox, eval=TRUE, fig.show = "hold", out.width = "48%", fig.cap="\\label{SimLevel_box} Simulation Level boxplots MP1=Economic, MP2=Standard Fisheries",  fig.align = "center", echo=FALSE}
...
ds2 %>%dplyr::filter(replicate>60)%>%dplyr::filter(mproc!=3) %>% ggplot(data=., aes(x=year, y=Simulated_SSB, group=mproc, color=mproc)) +
  geom_line() + 
...
ds2 %>%dplyr::filter(replicate>60)%>%dplyr::filter(mproc!=3) %>% ggplot(data=., aes(x=year, y=sumCW, group=mproc, color=mproc)) +
  geom_line() + 
  facet_wrap(vars(replicate,stock), scales="free_y", ncol=5)+
    theme(axis.text.y=element_blank(),  #remove y axis labels
        axis.ticks.y=element_blank(),  #remove y axis ticks
...
ds2 %>%dplyr::filter(replicate>60)%>%dplyr::filter(mproc!=3) %>% ggplot(data=., aes(x=year, y=ACL, group=mproc, color=mproc)) +
  geom_line() + 
  facet_wrap(vars(replicate,stock), scales="free_y", ncol=5)+
    theme(axis.text.y=element_blank(),  #remove y axis labels
        axis.ticks.y=element_blank(),  #remove y axis ticks
...
ds2 %>%dplyr::filter(replicate>60)%>%dplyr::filter(mproc!=3) %>% ggplot(data=., aes(x=year, y=F_full, group=mproc, color=mproc)) +
  geom_line() + 
  facet_wrap(vars(replicate,stock), scales="free_y", ncol=5)+
    theme(axis.text.y=element_blank(),  #remove y axis labels
        axis.ticks.y=element_blank(),  #remove y axis ticks
